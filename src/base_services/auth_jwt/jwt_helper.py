...
class ChromeExtensionJWTToken(BaseToken):
    ...
    def encode_token(self, user_id: int, email: str, has_staff_access: bool = False):
        payload = {
            ...
            "exp": datetime.datetime.utcnow() + datetime.timedelta(minutes=self.JWT_EXP_MINUTES),
...
class AuthJwtTokenService:
    ...
    def create_jwt_token(cls, request, **kwargs) -> Tuple[str, AuthRefreshToken, str]:
        ...
        access_token = ChromeExtensionJWTToken().encode_token(user_id=user.id, email=user.email, has_staff_access=True)
